...
const BorrowerDashboard = () => {
 ...
 <div>
 <h1 className="text-3xl md:text-4xl font-black text-text-primary italic tracking-tighter">Borrower Terminal</h1>
 <p className="text-sm md:text-base text-text-secondary0 font-medium">Manage your active obligations and protocol reputation.</p>
 </div>
 <div className="flex items-center gap-3 border border-border0/10 px-4 py-2 rounded-xl">
 <div className="w-2 h-2 rounded-full 0 animate-pulse"></div>
 <span className="text-[10px] font-black uppercase tracking-widest text-brand-accent0">Node Sync Active</span>
 </div>
 </header>
 ...
 <div className="flex items-center gap-3 text-text-secondary0 mb-4">
 <FiAward className="text-brand-accent0" />
 <span className="text-[9px] uppercase font-black tracking-widest">Trust Score</span>
 </div>
 <div className="text-5xl md:text-6xl font-black text-text-primary mb-1 tracking-tighter italic">{trustData.trustScore}</div>
 ...
 <div className="mt-2">
 <div className="flex justify-between text-[8px] text-text-primary font-bold mb-1">
 <span>Progress to ETH unlock</span>
 <span>{trustData.trustScore} / 700</span>
 </div>
 <div className="h-1.5 rounded-full overflow-hidden">
 <div
 className="h-full 0 rounded-full transition-all duration-700"
 ...
 <span className="text-[9px] text-text-secondary0 font-black uppercase tracking-widest">Completed Loans</span>
 <span className="text-lg font-black text-text-primary italic">{trustData.completedLoans}</span>
 </div>
 </div>

 <div className={`premium-card !p-8 border-l-4 ${hasIdentity ? 'border-l-emerald-500/50' : 'border-l-red-500/50'}`}>
 <div className={`flex items-center gap-3 mb-6 font-black uppercase tracking-widest text-[9px] ${hasIdentity ? 'text-brand-accent0' : 'text-brand-accent0'}`}>
 <FiShield /> Identity Status
 </div>
 <div className="flex items-center gap-4">
 <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center ${hasIdentity ? '0/5 text-brand-accent0' : '0/5 text-brand-accent0'}`}>
 {identityChecking ? <FiLoader className="animate-spin" /> : hasIdentity ? <FiCheckCircle size={24} /> : <FiInfo size={24} />}
 </div>
 <div className="overflow-hidden">
 <p className="text-text-primary font-black italic tracking-tight text-lg">
 {identityChecking ? 'Checking...' : hasIdentity ? 'Verified SBT' : 'Not Verified'}
 </p>
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-widest truncate">
 ...
 className="mt-6 w-full py-2 0/10 hover:0/20 text-brand-accent0 text-[8px] font-black uppercase tracking-widest rounded-lg transition-all"
 >
 ...
 <div className="premium-card !p-6 border-dashed border-border">
 <div className="flex items-center gap-2 mb-4 text-[8px] font-black uppercase tracking-widest text-text-primary">
 <FiActivity /> Protocol Health
 </div>
 <div className="space-y-3">
 <div className="flex justify-between items-center">
 <span className="text-[8px] text-text-secondary0 font-bold uppercase">Network</span>
 <span className="text-[8px] text-brand-accent0 font-black uppercase">{chainId === 11155111 ? 'Sepolia' : 'Wrong Network'}</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-[8px] text-text-secondary0 font-bold uppercase">Identity Node</span>
 <span className="text-[8px] text-text-secondary font-mono italic">{addresses.identity.slice(0, 6)}...{addresses.identity.slice(-4)}</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-[8px] text-text-secondary0 font-bold uppercase">Linked Identity</span>
 {(() => {
 ...
 return (
 <span className={`text-[8px] font-mono italic ${val === 'Checking...' ? 'text-text-secondary0' :
 !isAddr ? 'text-brand-accent' :
 isMatch ? 'text-brand-accent' : 'text-brand-accent'
 }`}>
 {val === 'Checking...' ? '...' :
 ...
 <h2 className="text-2xl md:text-3xl font-black text-text-primary italic tracking-tighter">Active Obligations</h2>
 <span className="text-[10px] text-text-secondary px-4 py-1.5 rounded-full font-black uppercase tracking-widest w-fit">{myLoans.length} Records</span>
 </div>
 ...
 {myLoans.map(loan => (
 <div key={loan._id} className="premium-card !p-8 md:!p-10 border-l-4 border-l-blue-600 group hover: transition-all duration-500">
 <div className="flex flex-col sm:flex-row justify-between gap-8 mb-10">
 <div className="space-y-2">
 <div className="flex items-center gap-3">
 <span className="text-[9px] md:text-[10px] font-mono text-text-secondary0 font-black uppercase tracking-widest">Proposal #{loan.simulatedSmartContractId || 'PENDING'}</span>
 <span className={`px-2 py-0.5 rounded-md text-[8px] md:text-[9px] font-black uppercase tracking-tighter ${loan.status === 'Funded' ? '0/10 text-brand-accent0' :
 loan.status === 'Repaid' ? '0/10 text-brand-accent0' :
 ' text-text-secondary0'
 }`}>
 {loan.status}
 </span>
 </div>
 <h3 className="text-4xl md:text-5xl font-black text-text-primary tracking-tighter italic">{loan.amountRequested} <span className="text-text-secondary0 text-sm font-normal not-italic ml-1">ETH</span></h3>
 </div>

 <div className="grid grid-cols-2 sm:flex sm:items-center gap-8 md:gap-12">
 <div className="space-y-1">
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-[0.2em]">Interest</p>
 <p className="text-text-primary font-black italic text-lg">{loan.interestRate}%</p>
 </div>
 <div className="space-y-1">
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-[0.2em]">Term</p>
 <p className="text-text-primary font-black italic text-lg">{loan.durationMonths}m</p>
 </div>
 <div className="col-span-2 sm:col-auto border-t sm:border-t-0 sm:border-l border-border pt-6 sm:pt-0 sm:pl-10 space-y-1">
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-[0.2em]">Total Settle</p>
 <p className="text-brand-accent0 font-black italic text-xl">{(loan.amountRequested * (1 + loan.interestRate / 100)).toFixed(4)}</p>
 </div>
 </div>
 </div>
 ...
 <div className="flex flex-col md:flex-row items-stretch md:items-end justify-between gap-8 pt-8 border-t border-border">
 <div className="flex-1 overflow-hidden">
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-[0.2em] mb-4">Blockchain Evidence</p>
 ...
 className="btn-primary w-full md:w-auto px-12 !py-4 whitespace-nowrap hover: text-[10px] font-black uppercase tracking-widest 0/10"
 >
 {processingLoan === loan._id ? <FiLoader className="animate-spin text-text-primary" /> : 'Settle On-Chain'}
 </button>
 )}
 </div>
 </div>
 ...
 <FiPlus className="text-brand-accent0" /> My Posted Loan Requests
 </h2>
 <p className="text-xs text-text-secondary0 font-medium mt-1">All ads you have broadcast on-chain to the lender marketplace.</p>
 </div>
 <div className="flex items-center gap-3">
 {myAdsLoading && <FiLoader className="text-brand-accent0 animate-spin" />}
 <span className="text-[10px] text-text-secondary px-4 py-1.5 rounded-full font-black uppercase tracking-widest w-fit">{myAds.length} Ad{myAds.length !== 1 ? 's' : ''}</span>
 </div>
 </div>
 ...
 {myAds.map(ad => (
 <div key={ad.adId} className={`premium-card !p-6 border-l-4 transition-all duration-300 hover: ${ad.status === 'Funded' ? 'border-l-emerald-500/60' : 'border-l-blue-500/50'}`}>
 {/* Ad ID + Status pill */}
 <div className="flex justify-between items-start mb-5">
 <span className="text-[9px] font-mono text-text-secondary0 font-black uppercase tracking-widest">Ad #{ad.adId}</span>
 <span className={`px-2.5 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${ad.status === 'Funded'
 ? '0/10 text-brand-accent border border-border0/20'
 : '0/10 text-brand-accent border border-border0/20'
 }`}>
 {ad.status}
 </span>
 ...
 {/* Principal */}
 <div className="mb-5">
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-widest mb-1">Principal</p>
 <p className="text-3xl font-black text-text-primary italic tracking-tighter">
 {Number(ad.principal).toFixed(4)}
 <span className="text-text-secondary0 text-sm font-normal not-italic ml-2">{ad.loanMode}</span>
 </p>
 </div>

 {/* Details grid */}
 <div className="grid grid-cols-2 gap-3 rounded-xl p-3 mb-5 border border-border">
 <div>
 <p className="text-[8px] text-text-secondary0 font-black uppercase tracking-widest mb-1">Total Repay</p>
 <p className="text-xs font-black text-text-primary">{Number(ad.totalRepayment).toFixed(4)} <span className="text-text-secondary0 font-normal">{ad.loanMode}</span></p>
 </div>
 <div>
 <p className="text-[8px] text-text-secondary0 font-black uppercase tracking-widest mb-1">Duration</p>
 <p className="text-xs font-black text-text-primary">{ad.repaymentInterval}m</p>
 </div>
 </div>
 ...
 <div className={`w-1.5 h-1.5 rounded-full ${ad.loanMode === 'ETH' ? '0' : '0'}`} />
 ...
 <div className="flex items-center justify-between mb-10">
 <div>
 <h2 className="text-2xl md:text-3xl font-black text-text-primary italic tracking-tighter">On-Chain Protocol Archive</h2>
 <p className="text-xs text-text-secondary0 font-medium mt-1">Direct cryptographic verification of your protocol interactions.</p>
 </div>
 {onChainLoading && <FiLoader className="text-brand-accent0 animate-spin" />}
 </div>
 ...
 <div className="w-1.5 h-1.5 rounded-full 0"></div> Created by You
 </h3>
 ...
 <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${loan.repaid ? '0/10 text-brand-accent0' : loan.funded ? '0/10 text-brand-accent0' : ' text-text-secondary0'}`}>
 ...
 <p className="text-[8px] font-black uppercase tracking-widest text-text-secondary0 mt-1">Interest: {loan.interest} ETH</p>
 </div>
 <div className="text-right">
 <p className="text-[8px] font-black uppercase tracking-[0.2em] text-text-secondary0 mb-1">Lender</p>
 <p className="text-[8px] font-mono text-text-secondary">{loan.lender === ethers.ZeroAddress ? 'OPEN MARKET' : `${loan.lender.slice(0, 6)}...${loan.lender.slice(-4)}`}</p>
 </div>
 </div>
 </div>
 ))
 ...
 <div className="flex items-center justify-between mb-10">
 <div>
 <h2 className="text-2xl md:text-3xl font-black text-text-primary italic tracking-tighter">Installment Agreements</h2>
 <p className="text-xs text-text-secondary0 font-medium mt-1">Active P2P loan contracts — pay monthly installments on-chain.</p>
 </div>
 {agreementsLoading && <FiLoader className="text-brand-accent0 animate-spin" />}
 </div>
 ...
 <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-lg ${agr.completed ? '0/10 text-brand-accent0' : agr.isOverdue ? '0/10 text-brand-accent0' : agr.isDue ? '0/10 text-brand-accent' : '0/10 text-brand-accent0'}`}>
 ...
 <div>
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-widest mb-1">Monthly Installment</p>
 <p className="text-3xl font-black text-text-primary italic tracking-tighter">
 {agr.monthlyPayment} <span className="text-text-secondary0 text-sm font-normal not-italic">{agr.mode === 0 ? 'ETH' : 'tUSDT'}</span>
 </p>
 </div>
 <div className="text-right">
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-widest mb-1">Total Remaining</p>
 <p className="text-lg font-black text-brand-accent italic tracking-tighter">
 ...
 <div>
 <p className="text-[8px] text-text-secondary0 font-black uppercase tracking-widest">Paid</p>
 <p className="text-text-primary font-black italic">{agr.paymentsMade}/{agr.totalDuration}</p>
 </div>
 <div className="border-x border-border">
 <p className="text-[8px] text-text-secondary0 font-black uppercase tracking-widest">Left</p>
 <p className="text-brand-accent font-black italic">{agr.remainingPayments}</p>
 </div>
 <div className="border-r border-border">
 <p className="text-[8px] text-text-secondary0 font-black uppercase tracking-widest">Missed</p>
 <p className="text-brand-accent font-black italic">{agr.missedPayments}</p>
 </div>
 <div>
 <div className="flex items-center justify-center gap-1">
 <FiCalendar size={9} className="text-text-secondary0" />
 <p className="text-[8px] text-text-secondary0 font-black uppercase tracking-widest">Due</p>
 </div>
 <p className="text-[9px] text-text-secondary font-bold">{agr.completed ? '—' : nextDue}</p>
 </div>
 </div>
 ...
 <p className="text-[8px] text-text-secondary0 font-black uppercase tracking-widest">Repayment Progress</p>
 ...
 className={`w-full btn-primary !py-4 text-[10px] font-black uppercase tracking-widest ${agr.isDue ? ' hover: 0/10' : 'opacity-50 cursor-not-allowed'}`}
 >
 ...
 {agr.completed && (
 <div className="flex items-center justify-center gap-2 py-4 0/5 border border-border0/20 rounded-xl">
 <FiCheckCircle className="text-brand-accent0" />
 <span className="text-[10px] font-black uppercase tracking-widest text-brand-accent0">Fully Repaid</span>
 </div>
 )}
 </div>
 );
 ...
 <div>
 <h2 className="text-2xl md:text-3xl font-black text-text-primary italic tracking-tighter flex items-center gap-3">
 <FiAward className="text-brand-accent0" /> Trust Score History
 </h2>
 <p className="text-xs text-text-secondary0 font-medium mt-1">Full audit trail of your reputation updates.</p>
 </div>
 ...
 ? '0/5 border-border0/15'
 : '0/5 border-border0/15'
 }`}
 >
 <div className="flex items-center gap-4">
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black ${entry.points >= 0 ? '0/10 text-brand-accent' : '0/10 text-brand-accent'
 }`}>
 {entry.points >= 0 ? '+' : ''}{entry.points}
 </div>
 <div>
 <p className="text-sm font-black text-text-primary">{entry.action}</p>
 <p className="text-[9px] text-text-secondary0 font-bold">
 ...
 <div className="text-right">
 <p className="text-[9px] text-text-secondary0 font-black uppercase tracking-widest mb-0.5">New Score</p>
 <p className="text-lg font-black text-text-primary italic">{entry.newScore}</p>
 </div>
 </div>
 ))}
 </div>
