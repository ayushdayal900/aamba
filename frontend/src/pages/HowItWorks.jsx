...
"Backend blockchain event poller syncs on-chain state to MongoDB every 15 seconds"
 ...
 const trustEvents = [
 { event:"NFT Minted", who:"Both", pts:"+300", },
 { event:"First loan funded", who:"Lender", pts:"+50", },
 { event:"Subsequent loans funded", who:"Lender", pts:"+10", },
 { event:"First repayment", who:"Borrower", pts:"+100", },
 { event:"Subsequent repayments", who:"Borrower", pts:"+75", },
 ];
 ...
 <p className="text-[10px] font-black uppercase tracking-[0.3em] text-text-primary">Protocol Documentation</p>
 <h1 className="text-4xl md:text-5xl font-black text-text-primary italic tracking-tighter">How PanCred Works</h1>
 <p className="text-text-secondary max-w-2xl mx-auto text-sm leading-relaxed font-medium">
 ...
 <div className={`w-12 h-12 flex-shrink-0 ${section.bg} ${section.color} rounded-2xl flex items-center justify-center text-xl group-hover:scale-110 transition-transform duration-300`}>
 {section.icon}
 </div>
 ...
 <h3 className="text-xl md:text-2xl font-black text-text-primary italic tracking-tight mb-4">{section.title}</h3>
 <ul className="space-y-2">
 {section.points.map((point, pi) => (
 <li key={pi} className={`flex items-start gap-2 text-sm ${point.startsWith('→') ? 'text-text-secondary ml-4' : 'text-text-secondary'} font-medium leading-relaxed`}>
 ...
 <div className="w-10 h-10 0/10 text-brand-accent0 rounded-xl flex items-center justify-center">
 <FiActivity />
 </div>
 <div>
 <p className="text-[9px] font-black uppercase tracking-widest text-text-primary">On-chain Registry</p>
 <h3 className="text-xl font-black text-text-primary italic">Trust Score Events</h3>
 </div>
 </div>
 ...
 <th className="text-left text-[9px] font-black uppercase tracking-widest text-text-primary pb-3">Event</th>
 <th className="text-left text-[9px] font-black uppercase tracking-widest text-text-primary pb-3">Who</th>
 <th className="text-right text-[9px] font-black uppercase tracking-widest text-text-primary pb-3">Points</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-slate-900/50">
 {trustEvents.map((e, i) => (
 <tr key={i} className="group hover: transition-colors">
 <td className="py-3 font-medium text-text-secondary">{e.event}</td>
 <td className="py-3 text-text-secondary0 font-medium">{e.who}</td>
 <td className={`py-3 text-right font-black italic ${e.color}`}>{e.pts}</td>
 </tr>
 ))}
 <tr className="border-t border-border">
 <td colSpan={3} className="pt-4 text-[10px] text-text-primary font-black uppercase tracking-widest">Max Score: 1000 pts · Capped per TrustScoreRegistry.sol</td>
 </tr>
 </tbody>
 </table>
 </div>
 ...
 >
 {[
 { tier:"Bronze Pilot", range:"0 – 99 pts", bg:"", border:"border-border", icon: <FiShield /> },
 { tier:"Silver Verified", range:"100 – 299 pts", bg:"", border:"border-border", icon: <FiAward /> },
 { tier:"Gold Elite", range:"300+ pts", bg:"", border:"border-border", icon: <FiStar /> },
 ].map((t, i) => (
 ...
 <p className="text-[10px] font-black uppercase tracking-widest text-text-primary mt-1">{t.range}</p>
 </div>
 ))}
 </motion.div>

 {/* CTA */}
 <div className="via-slate-900 border border-border rounded-3xl p-10 text-text-primary text-center">
 <h2 className="text-2xl md:text-3xl font-black italic tracking-tight mb-4">Ready to start your journey?</h2>
 <p className="text-text-secondary mb-8 max-w-xl mx-auto font-medium text-sm leading-relaxed">
