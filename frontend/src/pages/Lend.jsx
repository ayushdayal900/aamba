        ...
        <div className="space-y-8 md:space-y-12">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    ...
                    <p className="text-sm md:text-base text-fintech-muted font-medium italic">
                        Fund verified borrower requests. Earn monthly installment returns.
                    </p>
                </div>

                <div className="flex items-center gap-4">
                    <span className="text-[10px] bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-4 py-2 rounded-xl font-semibold uppercase tracking-wide">
                    ...
                    <button
                        ...
                        disabled={loading}
                        title="Refresh"
                        className="w-10 h-10 bg-fintech-dark hover:bg-slate-700 border border-fintech-border rounded-xl flex items-center justify-center text-fintech-muted hover:text-fintech-heading transition-all"
                    >
            ...
            {!hasFactory && (
                <div className="premium-card !p-8 border border-amber-500/20 bg-amber-500/5">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h3 className="text-fintech-heading font-semibold italic mb-2">Factory Contract Not Deployed</h3>
                            <p className="text-fintech-muted text-sm font-medium mb-4">
                                The LoanAgreementFactory contract has not been deployed yet. Run the deploy script first:
                            </p>
                            <code className="block bg-fintech-dark text-emerald-400 font-mono text-xs px-4 py-3 rounded-xl">
            ...
            {loading && hasFactory && (
                <div className="premium-card !p-12 flex flex-col items-center justify-center gap-4 text-fintech-muted">
                    ...
                    <p className="text-[11px] font-semibold uppercase tracking-wide">Syncing marketplace from Sepolia...</p>
                </div>
            )}
            ...
            {!loading && hasFactory && requests.length === 0 && (
                <div className="premium-card py-20 text-center space-y-6 border-2 border-dashed border-fintech-border">
                    ...
                    <div>
                        <p className="text-fintech-muted font-bold italic text-lg">No open loan requests detected.</p>
                        <p className="text-fintech-muted text-sm mt-2 font-medium">Borrowers haven't posted any requests yet, or all have been funded.</p>
                    </div>
                </div>
            )}
                    ...
                    {requests.map(request => (
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <span className="text-[9px] font-mono text-fintech-muted font-semibold uppercase tracking-wide">
                                        Request #{request.id}
                                    </span>
                                    <div className="flex items-center gap-2 mt-2">
                                        <div className="w-6 h-6 bg-fintech-accent/10 rounded-lg flex items-center justify-center">
                                            <FiUser size={12} className="text-fintech-accent" />
                                        </div>
                                ...
                                <span className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-3 py-1 rounded-lg text-[9px] font-semibold uppercase tracking-wide">
                            ...
                            {/* Principal */}
                            <div className="mb-6">
                                <p className="text-[9px] text-fintech-muted font-semibold uppercase tracking-wide mb-1">You Lend</p>
                                <p className="text-4xl font-bold text-fintech-heading tracking-tight">
                                    {request.principal} <span className="text-fintech-muted text-sm font-normal not-italic">{request.mode === 0 ? 'ETH' : 'tUSDT'}</span>
                                </p>
                            </div>

                            {/* Stats Grid */}
                            <div className="grid grid-cols-3 gap-4 mb-6 bg-fintech-dark/50 rounded-2xl p-4">
                                <div className="text-center">
                                    <div className="flex items-center justify-center gap-1 mb-1">
                                        <FiTrendingUp size={11} className="text-emerald-500" />
                                        <p className="text-[8px] text-fintech-muted font-semibold uppercase tracking-wide">You Earn</p>
                                    </div>
                                    <p className="text-emerald-400 font-black italic">{request.totalRepayment} {request.mode === 0 ? 'ETH' : 'tUSDT'}</p>
                                </div>
                                <div className="text-center border-x border-fintech-border">
                                    <div className="flex items-center justify-center gap-1 mb-1">
                                        <FiCalendar size={11} className="text-fintech-accent" />
                                        <p className="text-[8px] text-fintech-muted font-semibold uppercase tracking-wide">Duration</p>
                                    </div>
                                    <p className="text-fintech-heading font-semibold italic">{request.durationInMonths}m</p>
                                </div>
                                <div className="text-center">
                                    <div className="flex items-center justify-center gap-1 mb-1">
                                    ...
                                    <p className="text-fintech-heading font-semibold italic text-[11px]">{request.monthlyPayment} {request.mode === 0 ? 'ETH' : 'tUSDT'}</p>
                                </div>
                            </div>

                            {/* Insurance info */}
                            <p className="text-[9px] text-fintech-muted font-medium mb-6">
                                Insurance: {INSURANCE_FEE} ETH total distributed to treasury across installments.
                                Your net per month: <span className="text-fintech-muted font-black">
                            ...
                            <button
                                ...
                                className="w-full btn-primary !py-4 text-[10px] font-semibold uppercase tracking-wide bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-500/10 group-hover:shadow-emerald-500/20 transition-all"
                            >
                                {funding === request.id
            ...
            {/* Protocol note */}
            {hasFactory && (
                <div className="premium-card !p-8 bg-fintech-dark/40 border-dashed border-fintech-border flex items-start gap-4">
                    <FiCheckCircle className="text-emerald-500 shrink-0 mt-0.5" size={16} />
                    <div className="text-[10px] text-fintech-muted font-medium leading-relaxed space-y-1">
                        <p><strong className="text-fintech-muted">How it works:</strong> When you fund a request, a <code className="font-mono bg-fintech-dark px-1 rounded text-fintech-heading">LoanAgreement</code> contract is instantly deployed on Sepolia. The principal is forwarded directly to the borrower's wallet. You receive monthly installments minus a small insurance cut (0.01 ETH total per loan).</p>
